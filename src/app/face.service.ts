...
export class FaceService {
  ...
  async FaceDetection(
    ...
    this.results = await faceapi
      .detectAllFaces(video)
      .withFaceLandmarks()
      .withFaceDescriptors();
    const labeledDescriptors = this.results.map(
      (result: { descriptor: any }) => {
        return new faceapi.LabeledFaceDescriptors(registerNumber, [
          result.descriptor,
        ]);
      }
    );
  ...
  startInterval(
    ...
    return new Promise((resolve) => {
      this.timeInterval = setInterval(async () => {
        const labeledDescriptors = await this.FaceDetection(
          video,
          registerNumber,
          divTag
        );
  ...
  faceMatch(descriptorStored: any, descriptor: any): any {
    ...
    const labeledDes = new faceapi.LabeledFaceDescriptors(
      descriptorStored[0].label,
      [floatArray]
    );
